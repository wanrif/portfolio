...
const isLocalizedFile = (path: string): boolean =>
  path.endsWith('.en.mdx') || path.endsWith('.id.mdx');

export const getLocalizedModule = <TModule>(
...
export const getLocalizedModules = <TModule>(
  modules: Record<string, TModule>,
  locale: Locale,
): TModule[] => {
  const moduleEntries = Object.entries(modules);

  const localized = moduleEntries
...
export const stableSortByName = <TModule extends { meta: { name: string } }>(
  modules: TModule[],
): TModule[] => {
  const orderedModules: TModule[] = [];

  modules.forEach((entry) => {
    const insertionIndex = orderedModules.findIndex(
      (item) => item.meta.name.localeCompare(entry.meta.name) > 0,
    );
    if (insertionIndex === -1) {
